...
// 因为G0系列的单片机RAM有限，用户可以根据实际情况去修改此处缓冲区的大小，
...
HAL_StatusTypeDef LCD_SPI_Transmit(SPI_HandleTypeDef *hspi, uint16_t pData, uint32_t Size); // SPI传输函数
...
void HAL_SPI_MspInit(SPI_HandleTypeDef* hspi)
{
   ...
   if(hspi->Instance==SPI2)
   {
		...
		__HAL_RCC_GPIOD_CLK_ENABLE();    // 使能 SPI2 GPIO 
...
// 初始化 SCK、MOSI以及片选引脚，使用硬件 SPI 片选
      GPIO_InitStruct.Pin 		   = GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_4; // SCK、MOSI以及片选引脚
      GPIO_InitStruct.Mode 		= GPIO_MODE_AF_PP;            		// 复用推挽输出
      GPIO_InitStruct.Pull 		= GPIO_PULLUP;                		// 无上下拉
      GPIO_InitStruct.Speed 		= GPIO_SPEED_FREQ_VERY_HIGH;  		// 最高速度等级
      GPIO_InitStruct.Alternate  = GPIO_AF1_SPI2;              		// 复用到SPI2，复用线1
...
// SPI2 的内核时钟 = 64M，然后再经过2分频得到 32M 的SCK驱动时钟,此为 G070 所允许的最高速度
	...
	LCD_SPI.Init.CRCLength 						= SPI_CRC_LENGTH_DATASIZE;							// CRC校验长度，这里用不到	
	LCD_SPI.Init.NSSPMode 						= SPI_NSS_PULSE_ENABLE;      					   //	使用片选脉冲模式

   HAL_SPI_Init(&LCD_SPI);   // 初始化配置
}
